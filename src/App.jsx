...
import {
  ...
  Bell, UserPlus, Settings, Check, Globe, ArrowLeft, Image as ImageIcon, CornerUpLeft, ArrowDown, Palette, ArrowRight
} from "lucide-react";
          ...
          {authView === "landing" && (
            <div style={styles.authLanding}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                <DagsLogo size={38} />
                <span style={{ fontSize: 22, fontWeight: 900, letterSpacing: 1, color: T.textStrong }}>DAGS<span style={{ color: "#39FF88" }}>x19</span></span>
              </div>
              <h1 style={{ ...styles.authLandingTitle, color: T.textStrong }}>Arkadaşlarınla konuş.<br />Topluluğunu keşfet.<br /><span style={{ color: "#39FF88" }}>DAGSx19'a hoş geldin.</span></h1>
              <p style={{ ...styles.authLandingText, color: T.textDim }}>Sohbet et, yeni insanlarla tanış, ilgi alanlarını paylaş ve kendi topluluğunu oluştur.</p>

              <div style={styles.authLandingActions}>
                <button
                  style={styles.authLandingPrimary}
                  onClick={() => { setAuthView("register"); setAuthError(""); }}
                >
                  <span style={{ display: "flex", alignItems: "center", gap: 10 }}><UserPlus size={18} /><span>Hesap Oluştur</span></span>
                  <ArrowRight size={18} />
                </button>
                <button
                  style={{ ...styles.authLandingSecondary, borderColor: T.border, color: T.textStrong, background: T.panel }}
                  onClick={() => { setAuthView("login"); setAuthError(""); }}
                >
                  <span style={{ display: "flex", alignItems: "center", gap: 10 }}><Lock size={17} /><span>Giriş Yap</span></span>
                  <ArrowRight size={18} color={T.textDim} />
                </button>
              </div>

              <div style={styles.authLandingIconRow}>
                <div style={styles.authLandingIconItem}><MessageSquare size={18} color={T.textDim} /><span style={{ color: T.textDim }}>Sohbet</span></div>
                <div style={styles.authLandingIconItem}><Users size={18} color={T.textDim} /><span style={{ color: T.textDim }}>Arkadaşlar</span></div>
                <div style={styles.authLandingIconItem}><Server size={18} color={T.textDim} /><span style={{ color: T.textDim }}>Sunucular</span></div>
                <div style={styles.authLandingIconItem}><Shield size={18} color={T.textDim} /><span style={{ color: T.textDim }}>Güvenli</span></div>
              </div>

              <div style={{ ...styles.authLandingFoot, color: T.textDim }}>Zaten hesabın varsa <span style={styles.authLink} onClick={() => { setAuthView("login"); setAuthError(""); }}>giriş yap</span>.</div>
            </div>
          )}
    ...
    >
      <Trash2 size={12} />
          ...
          {Object.entries(rooms).map(([id, r]) => {
            ...
            const active = activeScope && scopeKey(activeScope) === scopeKey(sc);
            return <button key={id} style={{ ...styles.channelBtn, color: active ? "#39FF88" : T.textDim, background: active ? "#39FF8815" : "transparent" }} onClick={() => openScope(sc)}><Lock size={13} />{r.name}{hasUnread(sc) && <UnreadDot />}</button>;
          })}
  ...
  return (
    ...
    >
      ...
      {hasUnread(sc) && <UnreadDot />}
        ...
        <main style={{ ...styles.mainArea, ...(isMobile ? { display: view === "room" ? "flex" : "none", width: "100%", animation: view === "room" ? "slideInRight .16s ease" : "none" } : {}) }}>
          ...
          <div style={{ ...styles.modal, background: T.panel, borderColor: T.border, maxWidth: 420 }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={styles.panelList}>
              ...
              {Object.entries(publicServers).map(([id, s]) => (
                <div key={id} style={{ ...styles.panelRow, background: T.inputBg, borderColor: T.border }}>
...
sidebarSectionLabel: {
  ...
  authLanding: { width: "100%", maxWidth: 720, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", animation: "authCardIn .32s ease" },
  ...
  authLandingActions: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, width: "100%", maxWidth: 440, marginTop: 25 },
