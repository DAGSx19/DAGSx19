...
import {
  ...
  Bell, UserPlus, Settings, Check, Globe, ArrowLeft, Image as ImageIcon,
} from "lucide-react";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (record) setSettingsForm((prev) => ({ ...prev, note: record.note || "", bio: record.bio || "" }));
    })();
  }, [usernameLower]);
  ...
  const openProfile = async (username) => {
    ...
    setProfileCard({ username: record?.username || username, avatarColor: record?.avatarColor, avatarShape: record?.avatarShape, createdAt: record?.createdAt, note: record?.note, bio: record?.bio });
  };
  ...
  style={{
    ...
    color: T.text,
    ...(isMobile ? { display: view === "room" ? "none" : "flex", width: "100%" } : {}),
  }}
>
  ...
  return (
    ...
    >
      ...
      {hasUnread(sc) && <UnreadDot />}
        ...
        <main style={{ ...styles.mainArea, ...(isMobile ? { display: view === "room" ? "flex" : "none", width: "100%" } : {}) }}>
                      ...
                      <div style={{ ...styles.msgBubble, background: T.panel, borderColor: T.border }}>
                        ...
                        <div style={{ ...styles.reactionRow, position: "relative" }}>
                          {Object.keys(reactions).filter((emoji) => Object.keys(reactions[emoji] || {}).length > 0).map((emoji) => {
                            ...
                            return <button key={emoji} style={{ ...styles.reactionBtn, borderColor: mine ? "#39FF88" : T.border, color: mine ? "#39FF88" : T.textDim }} onClick={() => toggleReaction(m.id, emoji)}>{emoji} {count}</button>;
                          })}
                          <button style={{ ...styles.reactionBtn, borderColor: T.border, color: T.textDim, fontWeight: 700 }} onClick={() => setReactionPickerFor(reactionPickerFor === m.id ? null : m.id)}>+</button>
                          {reactionPickerFor === m.id && (
                            <div style={{ position: "absolute", bottom: "100%", left: 0, marginBottom: 4, display: "flex", flexWrap: "wrap", gap: 3, width: 190, padding: 6, borderRadius: 8, border: "1px solid", borderColor: T.border, background: T.panel, zIndex: 20 }}>
                              {EXTRA_EMOJIS.map((emoji) => (
                                <button key={emoji} style={{ background: "transparent", border: "none", fontSize: 16, cursor: "pointer", padding: 3 }} onClick={() => { toggleReaction(m.id, emoji); setReactionPickerFor(null); }}>{emoji}</button>
                              ))}
                            </div>
                          )}
                        </div>
